    ...
    void vmux_net::finalize() noexcept {
        ...
        if (NULL != tx_resolver) {
            vmux_post_exec(context_, strand_,
                [context = context_, strand = strand_, tx_resolver]() noexcept {
                    try {
                        tx_resolver->cancel();
                    }
    ...
    bool vmux_net::update() noexcept {
        ...
        return vmux_post_exec(context_, strand_,
            [self, this]() noexcept {
                ...
                for (SynchronizationObjectScope __SCOPE__(syncobj_);;) {
                    ...
                    else if (base_.established_ && (now - status_.last_heartbeat_) >= status_.heartbeat_timeout_) {
...
#pragma pack(pop)
        ...
        for (;;) {
            ...
            any |= vmux_post_exec(context_, strand_,
                [self, this, linklayer, buffer_memory, h, buffer_size]() noexcept {
                    return process_input(linklayer, h, buffer_size);
                });
        }
    ...
    bool vmux_net::process_input(const vmux_linklayer_ptr& linklayer, vmux_hdr* h, int length) noexcept {
        if (base_.disposed_) {
            return false;
        }

        uint64_t now = now_tick();
        if (packet_input_unorder(linklayer, h, length, now)) {
            return true;
        }
        else {
            close_exec();
    ...
    bool vmux_net::connect_yield(
        ...
        if (NULL == context || !connect_require(sk, host, port) || !y) {
            return false;
        }

        bool ok = false;
        vmux_post_exec(context_, strand_,
            [this, &sk, &host, &port, &ok, &y, &return_connection, &context, &strand]() noexcept {
                ok = connect(context, strand, sk, host, port,
